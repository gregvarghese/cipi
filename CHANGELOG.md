...
- **`cipi api status` appeared to hang after Domain** — Status prints **Queue** before Laravel version lines. **`php artisan`** / **`composer show`** / **`tinker`** use **`timeout`** when available; version lines fall back to **`unknown`** / **`dev`**, job count to **`?`**. New API installs run **`_api_ensure_psysh_home`** after setup. Servers **already on 4.4.16** before this revision can run **`cipi api fix-permissions`** once (migration does not re-run).
